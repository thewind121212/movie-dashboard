...
<template>
    ...
    <div
        class="w-screen h-screen flex justify-start items-center bg-[url('/images/background.png')] bg-cover bg-center relative pl-4 py-4 pr-4">
        <div class="w-full h-full absolute backdrop-blur-md top-0 left-0 z-10"></div>
        <div class="backdrop-blur-[120px] rounded-[20px] w-[16.5rem] overflow-hidden h-full bg-gradient-to-br from-[#060b26] to-[#1a1f37] bg-[linear-gradient(21.837897884493334deg, #060b26 0.00%, #1a1f37 100.00%)] p-4
            flex flex-col justify-start items-start gap-1 shadow-[0px_4px_6px_0px_rgba(0,0,0,0.02)] z-20  duration-300
            " :class="expanedSidebar ? 'w-[16.5rem]' : 'w-[4.5rem]'">
            <!-- logo  -->
            <div class="w-full h-auto flex justify-start items-center gap-3 relative">
            ...
            <!-- sperate -->
            <div class="w-full h-[1px] bg-[#c8c8c8] my-4"></div>
            <!-- home -->
            <h1 class="text-[1rem] text-white font-sans">Home</h1>
            ...
            <!-- user -->
            <h1 class="text-[1rem] text-white font-sans">Users</h1>
            <NavItem v-for="(item, index) in userDashboard" :key="index" :icon-path="item.iconPath" :name="item.name"
                :is-expanded="expanedSidebar" :url="item.url" />
            <!-- movie -->
            <h1 class="text-[1rem] text-white font-sans">Movie</h1>
        ...
        <div class="h-full z-20 rounded-[20px] ml-4 flex flex-col justify-start items-start gap-4 flex-1">
            <!-- header -->
            <div class="w-full h-[3rem] flex justify-between items-center">
                <div class="w-auto h-full bg-[#060b26] flex justify-center items-center px-4 rounded-lg">
                    <h2 class="font-[500] font-shatoshi text-[18px] text-[#FFFFFF] cursor-default"
